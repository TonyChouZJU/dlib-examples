...
int main(int argc, char** argv) try
{
    ...
    //std::ifstream train_file("/mnt/exhdd/tomorning_dataset/wonderland/raw_data/faces/famous_people/human_face/cluster_img_file_150pix.txt", ios::in);
    std::ifstream train_file("/mnt/exhdd/tomorning_dataset/wonderland/raw_data/faces/famous_people/enlarge/enlarge_cluster_img_file_250pix_2.txt", ios::in);
    string s;
    std::vector<string> img_file_vec;
    while(getline(train_file, s)) {
        string::size_type position = s.find_last_of(" ");
        if(position == s.npos)
            std::cout <<"not found"<< std::endl;
        else{
            img_file_vec.push_back(s.substr(0, position));
        }
    }
    ...
    for(int i=0; i <img_file_vec.size(); i++) {
        matrix<rgb_pixel> img;
        load_image(img, img_file_vec[i]);
        auto face_vector =  detector(img);
        if (face_vector.size() == 0)
        {
            cout << "No faces found in image!" << endl;
            continue;
        }
        auto shape = sp(img, face_vector[0]);
